...
def interate_students(lst):
    for i in range(len(lst)):
        for key, value in lst[i].items():
            print(f"{key} - {value}")
print(interate_students(students))
...
print(iterate_dictionary('last_name', students))
...
def print_dict_info(some_list):
    for key, value in some_list.items():
        print(f"{key} {len(value)}")
        for i in range(len(value)):
            print(value[i])
print(print_dict_info(dojo))